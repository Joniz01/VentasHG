...
const inp = (extra?: React.CSSProperties): React.CSSProperties => ({
  ...
  fontSize: 13, background: "var(--erp-bg)", color: "var(--erp-text)", width: "100%", ...extra,
});
...
export default function FacturaCompraForm({
  ...
  const [provSearched, setProvSearched] = useState(false); // si ya buscó y no encontró
  ...
  // ítems que el usuario pidió crear (sin ID)
  const [creatingProd, setCreatingProd] = useState<Record<number, boolean>>({});
  ...
  const handleImageFile = useCallback(async (file: File) => {
    try {
      ...
      setOcrError(null); setOcrLoading(true);
      try {
        ...
        // Pre-llenar proveedor desde OCR
  ...
  function canGoStep1() {
    return proveedorQ.trim().length > 0;
  }
  function canGoStep2() {
    const valid = items.filter((it) => it.nombreProducto.trim() && Number(it.cantidad) > 0);
    ...
    try {
      ...
      if (!finalProveedorId && proveedorQ.trim()) {
        const rp = await fetch("/api/proveedores", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nombre: proveedorQ.trim(), rifCi: proveedorRif.trim() || null, direccion: proveedorDir.trim() || null, telefono: proveedorTel.trim() || null }),
        });
      ...
      const resolvedItems = await Promise.all(validItems.map(async (it) => {
        ...
        if (!puedeCrearProducto) return it; // sin ID, se guarda solo el nombre
        // Crear producto
        try {
          const rp = await fetch("/api/productos", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nombre: it.nombreProducto.trim(), precio: 0, stockActual: 0, stockMinimo: 0 }),
          });
          const rd = await rp.json();
          if (rp.ok && rd.id) return { ...it, productoId: rd.id };
        } catch { /* ignorar, guardar sin ID */ }
        return it;
      }));
    ...
    <div style={{ maxWidth: 700 }}>
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", flex: i < STEPS.length - 1 ? 1 : "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{
                ...
                background: i < step ? "var(--erp-primary)" : i === step ? "var(--erp-primary)" : "var(--erp-border)",
                color: i <= step ? "#fff" : "var(--erp-text-3)",
              }}>
                {i < step ? "✓" : i + 1}
              </div>
            ...
            {i < STEPS.length - 1 && (
              <div style={{ flex: 1, height: 2, background: i < step ? "var(--erp-primary)" : "var(--erp-border)", margin: "0 6px", marginBottom: 16 }} />
            )}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="flex flex-col gap-4">
          <div style={{ background: "var(--erp-surface)", border: "1px solid var(--erp-border)", borderRadius: 12, padding: 16 }}>
            ...
            <div style={{ position: "relative" }}>
              <input
                ...
                placeholder="Escribe nombre o RIF del proveedor..."
                style={inp()}
                autoFocus
              />
              {showProvSugs && (
            ...
            {/* Proveedor nuevo — formulario de creación */}
            {provNuevo && provSearched && (
              <div style={{ marginTop: 12, background: "var(--erp-primary-lt)", border: "1px solid var(--erp-border)", borderRadius: 8, padding: 14 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--erp-primary)", marginBottom: 10 }}>
                  Proveedor nuevo — completa para crearlo al guardar
                </div>
                <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
                  ...
                  <div style={{ gridColumn: "1 / -1" }}>
                    ...
                    <input value={proveedorDir} onChange={(e) => setProveedorDir(e.target.value)} placeholder="Dirección" style={inp()} />
                  </div>
                  <div>
                    <label style={{ fontSize: 11, color: "var(--erp-text-3)", marginBottom: 3, display: "block" }}>Días de crédito</label>
                    <input type="number" min="0" value={proveedorDiasCredito} onChange={(e) => setProveedorDiasCredito(Number(e.target.value))} style={inp()} />
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="flex gap-3 justify-between">
            <button type="button" onClick={onCancel} style={{ background: "none", border: "1px solid var(--erp-border)", borderRadius: 8, padding: "9px 20px", fontSize: 13, cursor: "pointer", color: "var(--erp-text-2)" }}>Cancelar</button>
            <button type="button" onClick={() => setStep(1)} disabled={!canGoStep1()}
              style={{ background: "var(--erp-primary)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 24px", fontSize: 13, fontWeight: 700, cursor: canGoStep1() ? "pointer" : "not-allowed", opacity: canGoStep1() ? 1 : 0.5 }}>
              Siguiente →
            </button>
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="flex flex-col gap-4">
                    ...
                    <div className="flex gap-2 items-start">
                      <div style={{ flex: 2, position: "relative" }}>
                        ...
                        <input
                          ...
                          style={inp({ borderColor: sinId ? "#F59E0B" : undefined })}
                        />
                        {showProdSugs[it.key] && (
                      ...
                      <div style={{ width: 90, paddingTop: idx === 0 ? 20 : 0, textAlign: "right", fontSize: 13, fontWeight: 700, color: "var(--erp-text)", padding: idx === 0 ? "20px 4px 8px" : "8px 4px", fontVariantNumeric: "tabular-nums" }}>
                    ...
                    {sinId && (
                      <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 11, color: "#92400E", background: "#FEF3C7", borderRadius: 6, padding: "2px 8px" }}>
                          {puedeCrearProducto ? "⚠ Producto nuevo — se creará al guardar" : "⚠ Producto no encontrado en inventario (se guardará sin enlace)"}
                        </span>
                        {puedeCrearProducto && !creatingProd[it.key] && (
                          <span style={{ fontSize: 11, color: "var(--erp-text-3)" }}>stock inicial = 0</span>
                        )}
                      </div>
                    )}
                    {it.productoId && (
          ...
          <div className="flex gap-3 justify-between">
            <button type="button" onClick={() => setStep(0)} style={{ background: "none", border: "1px solid var(--erp-border)", borderRadius: 8, padding: "9px 20px", fontSize: 13, cursor: "pointer", color: "var(--erp-text-2)" }}>← Atrás</button>
            <button type="button" onClick={() => setStep(2)} disabled={!canGoStep2()}
              style={{ background: "var(--erp-primary)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 24px", fontSize: 13, fontWeight: 700, cursor: canGoStep2() ? "pointer" : "not-allowed", opacity: canGoStep2() ? 1 : 0.5 }}>
              Siguiente →
            </button>
          </div>
        </div>
      ...
      {step === 2 && (
        <div className="flex flex-col gap-4">
          ...
          <div style={{ background: "var(--erp-surface)", border: "1px solid var(--erp-border)", borderRadius: 12, padding: 14 }}>
            ...
            <div style={{ fontSize: 13, color: "var(--erp-text-2)" }}>
              <span style={{ fontWeight: 600, color: "var(--erp-text)" }}>{proveedorQ}</span>
            ...
            {items.some(it => it.nombreProducto.trim() && !it.productoId) && puedeCrearProducto && (
              <div style={{ fontSize: 11, color: "#92400E", marginTop: 4 }}>
                ⚠ {items.filter(it => it.nombreProducto.trim() && !it.productoId).length} producto(s) nuevo(s) se crearán en inventario
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 justify-between">
            <button type="button" onClick={() => setStep(1)} style={{ background: "none", border: "1px solid var(--erp-border)", borderRadius: 8, padding: "9px 20px", fontSize: 13, cursor: "pointer", color: "var(--erp-text-2)" }}>← Atrás</button>
            <button type="button" onClick={handleSubmit} disabled={saving}
              style={{ background: "var(--erp-primary)", color: "#fff", border: "none", borderRadius: 8, padding: "9px 28px", fontSize: 13, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
